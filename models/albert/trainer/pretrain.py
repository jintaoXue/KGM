...
class PretrainTrainer:
    def __init__(self, cfg, args=None):
        ...
        self.net = ALBERT(
            nb_in     = self.config['hidden_size'],
            seq_len   = self.config['max_len'], #seq_len,
            mlp_dim   = self.config['intermediate_size'], #mlp_dim,
            emb_dim   = self.config['embedding_dim'],
            nb_heads  = self.config['num_attention_heads'],
            nb_layers = self.config['num_hidden_layers'],
            nb_seg    = 2,
            dropout   = self.config['hidden_dropout_prob'],
            norm      = self.config['use_norm'],
            attention_type = 'softmax', #cfg.attention_type
            out_dim = self.config['out_size']
        ).to(self.device)
    ...
    def _calculate_loss(self, 
        ...
        cls_logits, token_logits = self.net(X, seg, mask = attention_mask) # input_ids, token_type_ids, attention_masks
    ...
    def train_step(self, batch):
        X, seg, attention_mask, token_labels, sentence_order = self._unpack_batch(batch) # seg is the token_type_ids
        self.net.train()
