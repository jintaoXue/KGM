...
class SimpleHGNConv(nn.Module):
    ...
    def __init__(self, edge_dim, in_dim, out_dim, num_heads, num_etypes, feat_drop=0.0,
                 negative_slope=0.2, residual=True, activation=F.elu, beta=0.0, aggregator_type='mean'):
        super(SimpleHGNConv, self).__init__()
        if aggregator_type == "lstm":
            self.lstm = nn.LSTM(
                out_dim*num_heads, out_dim*num_heads, batch_first=True, bidirectional = False
            )
        ...
        # mask out spatial relation type update
        # self.mask_type_id = mask_type_id
    def _lstm_reducer(self, nodes):
        ...
        m = m.view([batch_size, -1, self.out_dim*self.num_heads]) # (B, L, D)
        ...
        out, (rst, _) = self.lstm(m, h)
        return {"emb": rst.squeeze(0)}
    ...
    def forward(self, g, h, ntype, etype, presorted = False):
        ...
        with g.local_scope():
            ...
            g.srcdata['emb'] = emb
            # To do mask out spatial info update
            # mask = torch.where(g.ndata['_TYPE'] == 1, 0, 1)
            # sub_g = dgl.node_subgraph(g, torch.argwhere(mask).squeeze(-1).tolist())

            # sub_g_dst_emb = sub_g.dstdata['emb']
            # g_dst_emb = g.dstdata['emb']
