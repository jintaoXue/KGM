...
from utils.distributed import dist_statistics_scalar, get_rank
#NOTE{jintao} import SummaryWriter will cause some warinings but no probelm for using
...
class Logger:  # pylint: disable=too-many-instance-attributes
    ...
    def register_key(
        ...
        .. code-block:: text
            ...
            ----------------------------------------------------
            |    Train/Epoch        |             25           |
            |  Test/EpCost/Min   |            22.38         |
            |  Test/EpCost/Max   |            25.48         |
            |  Test/EpCost/Mean  |            23.93         |
            ----------------------------------------------------
...
def init_log(_cfgs):
    ...
    """
    _logger = Logger(
        output_dir=_cfgs.log_dir,
        exp_name=_cfgs.exp_name,
        seed=_cfgs.seed,
        use_tensorboard=_cfgs.logger_cfgs.use_tensorboard,
        use_wandb=_cfgs.logger_cfgs.use_wandb,
        config=_cfgs,
    )
    # if _logger._use_wandb:
    ...
    # if _cfgs.algo_cfgs.obs_normalize:
    #     obs_normalizer = self._env.save()['obs_normalizer']
    #     what_to_save['obs_normalizer'] = obs_normalizer
    _logger.setup_torch_saver(what_to_save)
    _logger.torch_save()

    # _logger.register_key('Test/precision', window_length=50)
    # _logger.register_key('Test/recall', window_length=50)
    # _logger.register_key('Test/f1', window_length=50)
    # _logger.register_key('Test/mean_precision', window_length=50)
    # _logger.register_key('Test/mean_recall', window_length=50)
    # _logger.register_key('Test/mean_f1', window_length=50)

    # _logger.register_key('Test/hit6', window_length=50)
    # _logger.register_key('Test/hit3', window_length=50)
    # _logger.register_key('Test/hit1', window_length=50)

    
    _logger.register_key('Train/hit6')
    _logger.register_key('Train/hit3')
    _logger.register_key('Train/hit1')
    _logger.register_key('Train/loss')

    # _logger.register_key("Test/step")
    # _logger.register_key('Test/hit6')
    # _logger.register_key('Test/hit3')
    # _logger.register_key('Test/hit1')

    # _logger.register_key('Train/LR')
    # if _cfgs.model_cfgs.actor_type == 'gaussian_learning':
    #     _logger.register_key('Train/PolicyStd')

    # _logger.register_key('TotalEnvSteps')

    # # log information about actor
    # _logger.register_key('Loss/Loss_pi', delta=True)
    # _logger.register_key('Value/Adv')

    # # log information about critic
    # _logger.register_key('Loss/Loss_reward_critic', delta=True)
    # _logger.register_key('Value/reward')

    # if _cfgs.algo_cfgs.use_cost:
    #     # log information about cost critic
    #     _logger.register_key('Loss/Loss_cost_critic', delta=True)
    #     _logger.register_key('Value/cost')

    # _logger.register_key('Time/Total')
    # _logger.register_key('Time/Rollout')
    # _logger.register_key('Time/Update')
    # _logger.register_key('Time/Epoch')
    # _logger.register_key('Time/FPS')
